  ...
  methods: {
    ...
    modification (formName) { // 修改
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const stuName = this.ruleForm.stuName // 学生姓名
          const stuClassId = this.selectFrame.classId // 班级编号
          const stuBirthDay = this.ruleForm.stuBirthDay // 生日
          const stuMobile = this.ruleForm.stuMobile // 手机号
          const stuPassword = this.ruleForm.stuPassword // 密码
          const stuSex = this.ruleForm.stuSex // 性别
          Api.ModifyStudent({
    ...
    addData (formName) { // 添加学生
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          const stuName = this.ruleForm.stuName // 学生姓名
          const stuClassId = this.ruleForm.className // 班级名称
          const stuBirthDay = this.ruleForm.stuBirthDay // 生日
          const stuMobile = this.ruleForm.stuMobile // 手机号
          const stuPassword = this.ruleForm.stuPassword // 密码
          const stuSex = this.ruleForm.stuSex // 性别
