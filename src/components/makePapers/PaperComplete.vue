...
export default {
  ...
  computed: {
    /** 选择题分数 */
    choiceScore() {
      return this.allPaperData.questions.filter(item => item.tpqQuestion.questionTypeId === 1).reduce((oldSum, num) => {return oldSum + num.tpqScore;}, 0);
    },
    /** 填空题分数 */
    blankScore() {
      return this.allPaperData.questions.filter(item => item.tpqQuestion.questionTypeId === 2).reduce((oldSum, num) => {return oldSum + num.tpqScore;}, 0);
    },
    /** 问答题分数 */
    essayScore() {
      return this.allPaperData.questions.filter(item => item.tpqQuestion.questionTypeId === 3).reduce((oldSum, num) => {return oldSum + num.tpqScore;}, 0);
    },
zf(){
  return this.allPaperData.questions.reduce((oldSum, num) => {return oldSum + num.tpqScore;}, 0);
}
  },
};
</script>
